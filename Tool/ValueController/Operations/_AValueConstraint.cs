// Copyright (c) 2025 Coda
// 
// This file is part of CodaGame, licensed under the MIT License.
// See the LICENSE file in the project root for license information.

using CodaGame.Base;

namespace CodaGame
{
    /// <summary>
    /// Base class for value constraints
    /// </summary>
    public abstract class _AValueConstraint<T_VALUE>
        where T_VALUE : struct
    {
        // The controller that owns this constraint.
        private _AValueController<T_VALUE> _m_controller;
        
        
        /// <summary>
        /// Update the constraint.
        /// </summary>
        protected abstract void Update(float _deltaTime);
        /// <summary>
        /// Evaluate the constraint.
        /// </summary>
        protected abstract T_VALUE Evaluate(T_VALUE _originalValue);
        
        
        /// <summary>
        /// Check if the constraint is added to a controller.
        /// </summary>
        /// <remarks>
        /// <para><paramref name="_controller"/> is optional. If provided, it checks if this constraint is added to that controller.</para>
        /// </remarks>
        internal bool IsConstraintAdded(_AValueController<T_VALUE> _controller = null)
        {
            if (_controller != null)
                return _m_controller == _controller;
            
            return _m_controller != null;
        }
        /// <summary>
        /// Set the constraint to be added to a controller.
        /// </summary>
        internal void SetConstraintAdded(_AValueController<T_VALUE> _aValueController)
        {
            if (_m_controller != null)
            {
                Console.LogWarning(SystemNames.ValueController, _m_controller.name, "Constraint already added to another controller");
                return;
            }

            _m_controller = _aValueController;
        }
        /// <summary>
        /// Set the constraint to be removed from a controller.
        /// </summary>
        internal void SetConstraintRemoved()
        {
            _m_controller = null;
        }
        /// <summary>
        /// Update the constraint.
        /// </summary>
        /// <remarks>
        /// <para>Just keeps the update function as the same as the behaviours'.</para>
        /// </remarks>
        internal void InternalUpdate(float _deltaTime)
        {
            Update(_deltaTime);
        }
        /// <summary>
        /// Evaluate the constraint.
        /// </summary>
        /// <remarks>
        /// <para>Just keeps the evaluate function as the same as the behaviours'.</para>
        /// </remarks>
        internal T_VALUE InternalEvaluate(T_VALUE _originalValue)
        {
            return Evaluate(_originalValue);
        }
    }
}